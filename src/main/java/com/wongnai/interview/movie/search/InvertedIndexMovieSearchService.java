...
import com.wongnai.interview.movie.Utils;
...
public class InvertedIndexMovieSearchService implements MovieSearchService {
    ...
    @Qualifier("invertedIndexMovieDatabase")
    @Autowired
    private HashMap<String, Set<Long>> index;
    ...
    public List<Movie> search(String queryText) {
        ...
        String[] queries = queryText.toLowerCase().split(" ");

        ArrayList<Long> intersected = new ArrayList<>(
                Arrays.stream(queries)
                        .filter(word -> index.containsKey(word))
                        .map(word -> index.get(word))
                        .reduce(Utils::intersect)
        ...
        return movieRepository.findByIdIn(intersected);
    }
}
